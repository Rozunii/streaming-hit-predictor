...
Las plataformas de streaming compiten por la atenci\'on del p\'ublico con cat\'alogos cada vez m\'as grandes. Decidir qu\'e contenido producir o adquirir implica inversiones significativas, y no todos los t\'itulos logran el nivel de audiencia esperado. El problema central es que las decisiones editoriales suelen basarse en criterios subjetivos o en la experiencia de quienes las toman, sin un respaldo sistem\'atico en datos hist\'oricos.

Este proyecto aborda ese problema construyendo un sistema que analiza el cat\'alogo de diez plataformas de streaming, con m\'as de 15{,}000 t\'itulos registrados entre 1980 y 2026. El sistema utiliza caracter\'isticas objetivas de cada t\'itulo, como el g\'enero, el a\~no de producci\'on, el presupuesto, las calificaciones del p\'ublico y la cr\'itica, y la cantidad de horas vistas, para identificar qu\'e factores est\'an asociados al \'exito comercial.

El objetivo es doble. Por un lado, describir el panorama actual del contenido en streaming mediante an\'alisis exploratorio y visualizaciones. Por otro, ofrecer una herramienta predictiva que permita estimar la probabilidad de \'exito de un t\'itulo antes de que sea lanzado, apoy\'andose en modelos de clasificaci\'on y agrupaci\'on entrenados con los datos hist\'oricos disponibles.
...
La conversi\'on de los datos crudos en informaci\'on \'util para los modelos se realiza en dos etapas secuenciales: limpieza y construcci\'on de variables. Cada regla est\'a motivada por una necesidad concreta del sistema.
...
El sistema se ejecuta como una secuencia de pasos con dependencias expl\'icitas. 

El pipeline se organiza en tres niveles de ejecuci\'on con dependencias expl\'icitas entre pasos.
...
La sabidur\'ia anticipada en la secci\'on inicial de este ensayo se concreta aqu\'i en decisiones espec\'ificas que el sistema construido hace posibles. El pipeline transforma datos crudos del cat\'alogo de streaming en modelos capaces de responder preguntas concretas de negocio con evidencia cuantitativa.
...
\subsection*{Descripci\'on del dashboard}

La navegaci\'on se organiza mediante un men\'u lateral con las siguientes p\'aginas:

\vspace{0.3cm}
\begin{center}
\begin{tabular}{p{4cm} p{10cm}}
\toprule
P\'agina & Descripci\'on \\
\midrule
\addlinespace[0.15cm]
Resumen del Dataset &
Estad\'isticas generales del cat\'alogo: distribuci\'on de t\'itulos por plataforma, g\'eneros m\'as frecuentes y cobertura temporal. \\
\addlinespace[0.3cm]
Preprocesamiento &
Muestra el resultado del pipeline de transformaci\'on: columnas derivadas, distribuci\'on de valores nulos y estad\'isticas del conjunto procesado. \\
\addlinespace[0.3cm]
Exploraci\'on (EDA) &
Visualizaciones de distribuciones, correlaciones entre variables y comparaciones por categor\'ias clave como tipo de contenido y g\'enero. \\
\addlinespace[0.3cm]
Comparaci\'on de Plataformas &
Tabla comparativa y gr\'afico de radar para evaluar dos plataformas en cuatro dimensiones: IMDb, Rotten Tomatoes, porcentaje de hits y premios ganados. \\
\addlinespace[0.3cm]
Predictor de \'Exito &
Formulario interactivo donde el usuario ingresa las caracter\'isticas de un t\'itulo y obtiene la predicci\'on de is\_hit con dos modelos (\'Arbol de Decisi\'on y SVM) m\'as la asignaci\'on de cluster K-Means. \\
\addlinespace[0.3cm]
Calidad Percibida &
Formulario similar orientado a predecir la categor\'ia IMDb (Bajo, Medio o Alto) usando la red neuronal. \\
\addlinespace[0.3cm]
Clusters de Contenido &
Mapa interactivo de los cuatro clusters identificados por K-Means, con descripci\'on del perfil de cada grupo y sus caracter\'isticas promedio. \\
\addlinespace[0.15cm]
\bottomrule
\end{tabular}
\end{center}

\subsection*{Pregunta de negocio principal}

La p\'agina de mayor valor para la toma de decisiones es el \textbf{Predictor de \'Exito Comercial}. Un analista puede ingresar las caracter\'isticas de un t\'itulo antes de adquirirlo, como plataforma, g\'enero, tipo, a\~no de lanzamiento, presupuesto, tama\~no del elenco y premios, y obtener en tiempo real la probabilidad de \'exito seg\'un dos modelos independientes. Cuando ambos modelos coinciden, la confianza en la decisi\'on aumenta; cuando difieren, el sistema lo notifica expl\'icitamente para que el analista considere el contexto adicional.

\subsection*{Visualizaci\'on de clusters}

El an\'alisis de agrupamiento revela la estructura latente del cat\'alogo. La figura siguiente muestra los valores promedio de las variables m\'as relevantes por cluster, lo que permite caracterizar cada perfil y entender qu\'e tipo de contenido lo compone.

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{figures/19_heatmap_clusters.png}
\caption{Mapa de calor de los cuatro clusters de contenido. Cada fila representa un grupo y cada columna una variable normalizada. Las diferencias entre perfiles son claras en t\'erminos de calificaci\'on, horas vistas y antig\"uedad del contenido.}
\end{figure}
